...
plt.plot(zeta+4*np.pi, np.sort(np.array(list0), axis=0)[:, 2], '-k', linewidth=3, label=r"field line")
#plt.show()
...
grid1 = eq._get_rtz_grid(np.array([1.]), np.array([0.]), zeta_full, coordinates="raz",
                        period=(np.inf, np.inf, np.inf), iota=iota)


ax = plt.gca()
#plt.plot(grid1.nodes[:, 2], -grid1.nodes[:, 1], '--', "#ff39ffff", linewidth=3, ms=2, label="umbilic edge")
plt.plot(grid1.nodes[:, 2], -grid1.nodes[:, 1], '--', color="purple", linewidth=3, ms=2, label="umbilic edge")
##plt.plot(np.mod(grid1.nodes[:, 2], 2*np.pi), grid1.nodes[:, 1], 'or', ms=2)

plt.xlabel(r"$\zeta$", fontsize=24, labelpad=-2)
plt.ylabel(r"$\theta$", fontsize=24, labelpad=-2)
