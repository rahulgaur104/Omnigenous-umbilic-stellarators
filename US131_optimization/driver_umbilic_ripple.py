...
for k in idx_list:
	...
	AspectRatio(eq=eq, bounds=(4, 10), weight=50), 
        RotationalTransform(eq=eq, grid=RotationalTransform_grid, bounds=(iota_sign*1/n-0.01, iota_sign*1/n+0.005),weight=iota_weight),
    ...
    eq_new.save(f"ripple_plots/eq_ripple_NFP{NFP_umbilic_factor}_{k}.h5")
    curve_opt.save(f"ripple_plots/curve_ripple_NFP{NFP_umbilic_factor}_{k}.h5")
    ...
    fig.write_html(f"ripple_plots/eq_and_coil_opt_ripple_NFP{NFP_umbilic_factor}_{k}.html")
    plt.close()

    fig, ax = plot_section(eq_new, name="|F|",norm_F=True, log=True)
    plt.savefig(f"ripple_plots/F_norm_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()

    fig, ax = plot_comparison(eqs=[eq])
    plt.savefig(f"ripple_plots/Xsection_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)

    fig, ax = plot_boozer_surface(eq, rho=1)
    plt.savefig(f"ripple_plots/Boozer_eq_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()

    fig, ax = plot_boozer_surface(eq, rho=0.5)
    plt.savefig(f"ripple_plots/Boozer_eq_ripple_0p5_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()
