...
A = n * theta_arctan - m * phi

arr = np.array([phi, A]).T
data_curve[:, :] = arr 

curve = FourierUmbilicCurve.from_values(coords=jnp.array(data_curve), N=15, NFP=m, NFP_umbilic_factor=NFP_umbilic_factor, sym="sin")
curve0 = curve.copy()
...
for k in idx_list:
    ...
    objective = ObjectiveFunction(
        (
        ForceBalance(eq=eq, weight=2e5),
        UmbilicHighCurvature(eq=eq, curve=curve, bounds=(-85, -38), weight=1),
	AspectRatio(eq=eq, bounds=(4, 10), weight=50), 
        RotationalTransform(eq=eq, grid=RotationalTransform_grid, bounds=(iota_sign*1/n+0.00, iota_sign*1/n+0.005),weight=iota_weight),
        RotationalTransform(eq=eq, grid=RotationalTransform_grid2, bounds=(iota_sign*np.inf, iota_sign*0.25),weight=iota_weight/4),
        Elongation(eq=eq, grid=Elongation_grid, bounds=(0.5, 3.0), weight=2e2), 
        Shear(eq=eq, grid=RotationalTransform_grid, bounds=(-np.inf, -0.05),weight=1e3),
    ...
    constraints = (
        ...
        #LinearObjectiveFromUser(mirrorRatio, field, target=[0.35, 1.5]),
    )
    ...
    if k <= 4:
        (eq_new, curve_opt), _ = optimizer.optimize(
            things=(eq, curve), objective=objective, constraints=constraints, verbose=2, maxiter=45, ftol=1e-3, copy=True
        )
    else:
        (eq_new, curve_opt), _ = optimizer.optimize(
            things=(eq, curve), objective=objective, constraints=constraints, verbose=2, maxiter=22, ftol=1e-3, copy=True
        )

    eq = eq_new.copy()
    curve = curve_opt.copy()

    eq_new.save(f"ripple_plots3/eq_ripple_NFP{NFP_umbilic_factor}_{k}.h5")
    curve_opt.save(f"ripple_plots3/curve_ripple_NFP{NFP_umbilic_factor}_{k}.h5")


    NL = 51
    grid0 = LinearGrid(L = NL, M=0, N=0)
    rho = np.linspace(0, 1, NL+1)
    print(eq.compute("iota", grid=grid0)["iota"])
    ...
    fig.write_html(f"ripple_plots3/eq_and_coil_opt_ripple_NFP{NFP_umbilic_factor}_{k}.html")
    plt.close()

    fig, ax = plot_section(eq_new, name="|F|",norm_F=True, log=True)
    plt.savefig(f"ripple_plots3/F_norm_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()

    fig, ax = plot_comparison(eqs=[eq])
    plt.savefig(f"ripple_plots3/Xsection_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)

    fig, ax = plot_boozer_surface(eq, rho=1)
    plt.savefig(f"ripple_plots3/Boozer_eq_ripple_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()

    fig, ax = plot_boozer_surface(eq, rho=0.5)
    plt.savefig(f"ripple_plots3/Boozer_eq_ripple_0p5_NFP{NFP_umbilic_factor}_{k}.png", dpi=400)
    plt.close()
    ...
    data = eq.compute(
            ...
            theta = Bounce2D.compute_theta(eq, 64, 64, rho=rho_ripple),
            Y_B=128,
            num_transit=num_transit,
