...
for i in range(4):
    plt.plot(data1["R"][:, 1, i], data1["Z"][:, 1, i], linecolor[i], linewidth=2, label=labels[i])
...
plt.savefig("boundary_plots/boundary_comparison.pdf", dpi=300)
#plt.savefig("boundary_plots/boundary_comparison.png", dpi=300)
plt.show()
plt.close()
