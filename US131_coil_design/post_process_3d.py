...
#plot_grid = LinearGrid(rho=1.0, M=40, N=40, NFP=1, sym=True)
fig = plot_3d(eq,"B*n", field=field, grid=plot_grid)
...
for i, coil in enumerate(coil_traces):
    ...
    if i < 16:
        ...
        tube_mesh = create_tube_mesh(x, y, z, radius=0.00000001, n_segments=16)
        ...
        tube_mesh = create_tube_mesh(x, y, z, radius=0.005, n_segments=16)
        # Add the tube as a mesh3d trace
        new_fig.add_trace(go.Mesh3d(
            x=tube_mesh['vertices'][:, 0],
            y=tube_mesh['vertices'][:, 1],
